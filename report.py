...
# ========== 日志配置 ==========
LOG_FILE = '/var/log/aliyun_daily.log'   # 日志路径
logging.basicConfig(
    level=logging.INFO,
...
def send_tg_report(tg_conf, message):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.error(f"Telegram 消息发送失败，HTTP状态码: {resp.status_code}, 响应: {resp.text}")
    except Exception as e:
        logger.exception(f"Telegram 发送异常: {e}")
...
def do_common_request(client, domain, version, action, params=None, method='POST', timeout=30, retries=3):
    for attempt in range(1, retries + 1):
        try:
            ...
            request.set_connect_timeout(5000)   # 连接 5 秒
...
def main():
    ...
    for idx, user in enumerate(users, 1):
        try:
            ...
            # 1. CDT 流量
            traffic_data = do_common_request(
                AcsClient(user['ak'].strip(), user['sk'].strip(), 'cn-hangzhou'),
                'cdt.aliyuncs.com', '2021-08-13', 'ListCdtInternetTraffic'
            )
            traffic_gb = -1
            if traffic_data:
            ...
            # 记录每个用户的报告详情
            logger.info(f"用户 [{user_name}] 报告详情:\n{user_report}")
            report_lines.append(user_report)

        except Exception as e:
            error_msg = f"处理用户 {user.get('name', 'Unknown')} 时出错: {str(e)}"
